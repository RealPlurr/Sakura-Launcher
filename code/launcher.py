...
def config_window():
    ...
    ConfigWindow.title("SakuraLauncher Config")  # sets window title
...
def play():
    ...
    # NonPremium code
    if Email == "":
        warning_box("SakuraLauncher Error!", "Username cannot be empty!")
    else:
        if Config.Config["Premium"]:
        ...
        # message_box("SakuraLauncher", "Thank you for using SakuraLauncher!") #Temporarily disabled as it would create an empty tk window
...
def config_load():
    ...
    if "LastSelected" not in list(Config.Config.keys()):
        Config.Config["LastSelected"] = "1.12.2"
...
if __name__ == "__main__":
    ...
    MainWindow.title("SakuraLauncher")  # sets window title
    if SYSTEM == "Windows":
        # other systems dont use ico
        MainWindow.iconbitmap(constants.ui.LOGO_ICON)  # sets window icon
    ...
    PInfo_Label = Label(
        MainWindow,
        text=f"SakuraLauncher {Config.Version}",
        bg=constants.ui.BG_COLOUR,
        fg="white",
        font="Arial 15 bold",
    )
